...
export default function RestaurantsScreen ({ navigation, route }) {
  ...
  const renderRestaurant = ({ item }) => {
    return (
      ...
      >
        <TextSemiBold style={{ fontSize: 15, fontFamily: 'Montserrat_600SemiBold' }}>{item.name} {item.discount && item.discountCode && <TextSemiBold textStyle={{ color: GlobalStyles.brandPrimary }}>({item.discount}% off with code {item.discountCode})</TextSemiBold>}</TextSemiBold>
